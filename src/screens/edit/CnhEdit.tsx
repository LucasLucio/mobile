...
export function CnhEdit({route, navigation}) {
  ...
  function loadDados() {
    if (dados) {
        setValue("cnh", dados.pessoa.pessoaFisica.motorista.cnh); 
        setValue("categoria", dados.pessoa.pessoaFisica.motorista.categoria); 
        setValue("dataEmissao", dados.pessoa.pessoaFisica.motorista.dataEmissao); 
        setValue("dataValidade", dados.pessoa.pessoaFisica.motorista.dataValidade); 
        setDataEmissao(new Date(dados.pessoa.pessoaFisica.motorista.dataEmissao))
        setDataValidade(new Date(dados.pessoa.pessoaFisica.motorista.dataValidade))
    }
  }
  useEffect(() => {
    loadDados();
  ...
  async function cnh(data){
    accountEdit.cnh = data.cnh;
    accountEdit.categoria = data.categoria;
    accountEdit.dataEmissao = dayjs(dataEmissao.toLocaleString()).format('DD/MM/YYYY');
    accountEdit.dataValidade = dayjs(dataValidade.toLocaleString()).format('DD/MM/YYYY');

    navigation.navigate('EnderecoEdit', {accountEdit, dados})
  }
